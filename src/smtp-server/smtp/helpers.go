...
func StoreEmail(request * Request) (error) {
	from := request.From
	path := "./" + from
	...
	filename := path + "/" + from + time.Now().Format("2006-01-02-15-04-05") +  ".eml"
	
	file,_ := os.Create(filename)
	defer file.Close()

	message := StreamToString(request.Message)

	_,_ = file.Write([]byte(message))
